...
namespace xe {
    ...
    void BlinnPhongMaterial::init() {
        create_material_uniform_buffer(4 * sizeof(glm::vec4) + 2 * sizeof(int));
    ...
    void BlinnPhongMaterial::bind() const {
        ...
        OGL_CALL(glBufferSubData(GL_UNIFORM_BUFFER, sizeof(glm::vec4), sizeof(glm::vec4), &Ka_));
        OGL_CALL(glBufferSubData(GL_UNIFORM_BUFFER, 2 * sizeof(glm::vec4), sizeof(glm::vec4), &Kd_));
        OGL_CALL(glBufferSubData(GL_UNIFORM_BUFFER, 3 * sizeof(glm::vec4), sizeof(glm::vec4), &Ks_));
        OGL_CALL(glBufferSubData(GL_UNIFORM_BUFFER, 4 * sizeof(glm::vec4), sizeof(int), &use_vertex_colors_));
        if (texture_ > 0) {
            use_map_kd_ = 1;
            OGL_CALL(glBufferSubData(GL_UNIFORM_BUFFER, 4 * sizeof(glm::vec4) + sizeof(int), sizeof(int), &use_map_kd_));
